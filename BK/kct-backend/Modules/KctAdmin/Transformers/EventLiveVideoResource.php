...
/**
 * @OA\Schema(
 *  title="APIResource: EventLiveVideoResource",
 ...
 *  @OA\Property( property="video_type",type="string",description="Type of the video(YouTube/Vimeo)",example="1"),
 ...
 * ---------------------------------------------------------------------------------------------------------------------
 *
 * Class EventLiveVideoResource
 *
 * @package Modules\KctAdmin\Transformers
 */
class EventLiveVideoResource extends JsonResource {
    use ServicesAndRepo;
    ...
    public function toArray($request): array {
        return [
            ...
            'value'         => $this->resource['path'] ?? ($this->resource['value'] ?? null),
            'video_type'    => $this->resource['type'] ?? ($this->resource['video_type'] ?? null),
            'thumbnail_path' => $this->adminServices()->fileService->getFileUrl($this->resource['thumbnail_path']),
        ];
    }
}
