...
class StockService extends Service {
    ...
    private function addFileNameToPath($path, $url) {
     ...
     * @throws CustomValidationException
     */
    private function getImageByUrl($imageUrl) {
     ...
     * @return Image
     */
    private function cropImage($request, $image) {
     ...
     * @return mixed
     */
    private function resizeImage($image, $width) {
     ...
     * @return string
     */
    private function uploadImageToS3($image, $path, $visibility) {
        $image->stream();
    ...
    public function isStockAvailable() {
        $tenancy = app(\Hyn\Tenancy\Environment::class);
        $setting = AccountSettings::where('account_id', $tenancy->hostname()->id)->first()->setting;
        return isset($setting['stock_setting']['enabled']) && $setting['stock_setting']['enabled'];
    }
