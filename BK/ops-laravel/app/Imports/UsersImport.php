<?php
     ...
     * @package App\Imports
     */
    class UsersImport implements ToCollection, WithValidation, WithStartRow, WithMultipleSheets
    {
        use Importable;
...
//                                $containsIns[$key][$k] = strtolower($row[$item]);
                            ...
                            foreach ($default as $k => $item) {
                                if ($k == 'email') {
                                    ...
                                    $ins[$key]['password'] = bcrypt(strtolower($row[$item]));
                                    $ins[$key]['import_email'] = 0;
                                    $ins[$key]['role'] = 'M2';
...
//                                unset($containsIns[$contain]['email']);
                        ...
                        if ($this->listId > 0) {
                            ...
                            $count = count($ids);
                            $checkAlready = IcontactMeta::whereIn('column_id', array_column($containsIns, 'id'))->get(['column_id', 'icontact_id', 'created_at']);
                            $already = [];
                        ...
                        if (count($cusIns) > 0 || $cusContainsIns > 0) {
                            $importService->addUserSkill($insertedIds, $cusIns, 0, $cusContainsIns);
                        }

                        if (count($eniIns) > 0 || count($eniContainsIns) > 0) {
                            $importService->addPersonEntity($insertedIds, $eniIns, 0, $entityPosIns, $eniContainsIns, $entityPosContainsIns, $entityPosTypeIns, $entityPosTypeContainsIns, $entityPosType);
                        }
