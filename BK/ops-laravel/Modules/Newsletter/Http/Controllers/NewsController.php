    ...
    public function store(NewsCreateRequest $request) { // create news
        try {
            DB::connection('tenant')
                ->beginTransaction();// to provide the tenant environment and transaction will only apply to model which extends tenant model
            $param = [
                'title'              => $request->title,
...
//connection()->
    ...
    public function newsStatusCount() {
        try {
            ...
            } else if ($workshop = Workshop::with(['meta' => function ($q) {
