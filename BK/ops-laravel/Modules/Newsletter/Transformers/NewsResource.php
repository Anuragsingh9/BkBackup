...
class NewsResource extends Resource {
    ...
    public function dates() {
        if (in_array($this->status, ['validated', 'sent'])) {
            return $this->mergeWhen(in_array($this->status, ['validated', 'sent']), ['validated_on' => $this->validatedOn->first() ? Carbon::parse($this->validatedOn->first()->fields['validated_on']['date'])
                ->format('Y-m-d') : null]);
        }
        return $this->mergeWhen(in_array($this->status, ['rejected']), ['rejected_on' => $this->validatedOn->first() ? Carbon::parse($this->validatedOn->first()->fields['rejected_on']['date'])
            ->format('Y-m-d') : null]);
    }
