<?php
    ...
    class CreateSkillUnionView extends Migration
    {
        /**
         * Run the migrations.
         *
        ...
        {
            DB::statement("
      CREATE OR REPLACE VIEW skill_union_view AS
      (
     SELECT
...
FROM
    `user_skills`
WHERE
    `user_skills`.`field_id` = `entities`.`id` AND `user_skills`.`skill_id` = `skill_data`.`skill_id` AND `user_skills`.`type` = 'union'
GROUP BY
...
WHERE
    `skill_data`.`tab_type` = 4 AND `entities`.`entity_type_id` = 3
      )
    ");
        }
        ...
        {
            DB::statement("DROP VIEW skill_union_view");
        }
    }
