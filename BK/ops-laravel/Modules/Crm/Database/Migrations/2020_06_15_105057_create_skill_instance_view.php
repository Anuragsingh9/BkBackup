<?php
    ...
    class CreateSkillInstanceView extends Migration
    {
        /**
         * Run the migrations.
         *
        ...
        {
            DB::statement("
      CREATE OR REPLACE VIEW skill_instance_view AS
      (
       SELECT
...
FROM
    `user_skills`
WHERE
    `user_skills`.`field_id` = `entities`.`id` AND `user_skills`.`skill_id` = `skill_data`.`skill_id` AND `user_skills`.`type` = 'instance'
GROUP BY
...
WHERE
    `skill_data`.`tab_type` = 3 AND `entities`.`entity_type_id` = 1
      )
    ");
        ...
        function down()
        {
            DB::statement("DROP VIEW skill_instance_view");
        }
    }
