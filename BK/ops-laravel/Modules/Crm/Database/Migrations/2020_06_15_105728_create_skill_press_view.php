...
class CreateSkillPressView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::statement("
      CREATE OR REPLACE VIEW skill_press_view AS
      (
     SELECT
...
FROM
    `user_skills`
WHERE
    `user_skills`.`field_id` = `entities`.`id` AND `user_skills`.`skill_id` = `skill_data`.`skill_id` AND `user_skills`.`type` = 'press'
GROUP BY
...
WHERE
    `skill_data`.`tab_type` = 7 AND `entities`.`entity_type_id` = 4
      )
    ");
    }
    ...
    {
        DB::statement("DROP VIEW skill_press_view");
    }
}
