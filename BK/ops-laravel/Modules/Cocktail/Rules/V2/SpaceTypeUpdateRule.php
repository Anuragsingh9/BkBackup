...
class SpaceTypeUpdateRule implements Rule
{
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($isPassDefaultValidation){
                $this->msg  = __('cocktail::message.default_not_possible');
                return false;
        }
        // Getting the space type of given space
        $currentSpaceType = $this->getSpaceType($this->space);
        switch ($currentSpaceType){
            case config('cocktail.default.space_type_vip'):
                $error =  $this->validateVip($value);
                break;
                case config('cocktail.default.space_type_duo'):
    ...
    public function validateDefaultSpace($space_uuid,$value){
        ...
        if($defaultSpace->space_uuid === $this->space->space_uuid){
            ...
            if ($value != 0){
                return __('cocktail::message.default_not_possible');
            }
                return "";
        }
    }
    ...
    public function validateVip($value){
        if($value == 2){
            ...
            return __('cocktail::message.vip_to_duo_not_possible');
        }
            return "";
    }
    ...
    public function validateDuo($value){
        if($value != 2){
            ...
            return __('cocktail::message.duo_change_not_possible');
        }
            return "";
    }
    ...
    public function validateRegular($value){
        if($value == 2){
            ...
            return __('cocktail::message.regular_to_duo_not_possible');
        }
            return "";
    }
