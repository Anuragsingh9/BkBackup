...
class SpaceTypeRule implements Rule {
    ...
    public function __construct($uuid, $source) {
        if ($source == 'event') {
            ...
            $space = ValidationV2Service::getInstance()->resolveSpace($uuid);
            if ($space) {
    ...
    public function passes($attribute, $value) {
        if ($this->eventUuid) {
            ...
            if ($value == config('cocktail.default.space_type_duo')) {
                if(KctCoreService::getInstance()->findEventConferenceType($this->eventUuid)) {
                    $this->msg = __('cocktail::message.conference_cannot_have_duo');
                    return false;
                }
                $result = ValidationV2Service::getInstance()->isDuoCreated($this->eventUuid, $this->spaceUuid);
                if ($result) {
                    $this->msg = __('cocktail::message.one_duo_possible');
                    return false;
                }
            }
        }
